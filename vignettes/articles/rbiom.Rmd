...
```{r}
biom$metadata$treatment <- sample(c("control", "test"), biom$n_samples, TRUE)
biom$metadata
```
...
rbiom object. The base R `subset()` function works well for this, as do the 
dplyr verbs `dplyr::filter()` and `dplyr::slice()`.

```{r}
biom <- subset(biom, `Body Site` == "Anterior nares")
biom$metadata
biom
```
