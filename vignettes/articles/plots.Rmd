...
| Stacked Bar Plots | `taxa_barplot()`                                      |
...
biom <- rarefy(hmp50)
patchwork::wrap_plots(lapply(
  c("trend", "confidence", "scatter", "residual", "name"), 
  FUN = function (layer) {
...
Unambiguous abbreviations are also accepted. For instance, 
`layers = c("trend", "scatter")` is equivalent to `layers = c("t", "s")` and 
`layers = "ts"`.
...
```{r}
taxa_barplot(rarefy(hmp50), taxa = 10)
```
