...
cat(glue::glue('
  <table class="table-borderless">
  <tr>{adiv_stats}{bdiv_stats}{distmat_stats}{stats_table}{taxa_stats}</tr>
  </table>',
  .transformer = function (text, ...) {
    sprintf("<td><code><a href='../reference/%s.html'>%s()</a></code></td>", text, text)
  }))
...
The `test` parameter for these plots allows you to control what statistical 
question is being addressed.

| Test        | Function              | Question                                       |
| ----------- | --------------------- | ---------------------------------------------- |
| `fit`       | `broom::glance.lm()`  | Does the model (e.g. y ~ x) fit the data well? |
| `trends`    | `emmeans::emtrends()` | Is each trendline's slope non-zero?            |
| `pw_trends` | `emmeans::emtrends()` | Do pairs of trendlines have different slopes?  |
| `means`     | `emmeans::emmeans()`  | Is each trendline's mean non-zero?             |
| `pw_means`  | `emmeans::emmeans()`  | Do pairs of trendlines have different means?   |


These are parametric tests. To apply them to alpha diversity, beta diversity, 
or taxa abundance non-parametric data, rbiom defaults to using the rank 
transformation, e.g. `adiv_corrplot(biom, trans="rank", ...)`. Therefore, the 
y-axis title for correlation plots will be something similar to 
"rank(Shannon Diversity)" and the `$data` plot attribute will contain the rank- 
transformed Shannon diversity values.


Generally, more than one p-value will be generated per plot, for instance, one 
p-value per trendline or one p-value per pair of trendlines. Showing all the 
p-values on the plot can get crowded, so instead the plot displays "min(p)", 
the most significant p-value after correcting for multiple corrections. The 
caption under the plot provides a quick reminder of what is being tested for.
...
p <- taxa_corrplot(
  biom = biom, taxa = "Bacteroides", layers = "tcs", 
  x = "Age", color.by = "Body Site")
...
Using the corresponding `_stats()` function will generate the same statistics.
...
taxa_stats(
  biom = biom, taxa = "Bacteroides", 
  regr = "Age", stat.by = "Body Site")
```
