...
#'     library(rbiom)
#'     
#'     taxa.sums(hmp50, 'Genus') %>% head(10)
#'

taxa.sums <- function (biom, rank = NULL) {
  
  if (!is(biom, 'BIOM'))
    stop (simpleError('In taxa.sums(), biom must be a BIOM-class object.'))
  
  if (is.null(rank)) {
    x <- slam::row_sums(biom$counts)
  } else {
    x <- taxa.rollup(biom, rank) %>% colSums() %>% sort(decreasing = TRUE)
  }
...
#'     library(rbiom)
#'     
#'     taxa.means(hmp50, 'Genus') %>% head(10)
#'

taxa.means <- function (biom, rank = NULL) {
  
  if (!is(biom, 'BIOM'))
    stop (simpleError('In taxa.means(), biom must be a BIOM-class object.'))
  
  if (is.null(rank)) {
    x <- slam::row_means(biom$counts)
  } else {
    x <- taxa.rollup(biom, rank) %>% colMeans() %>% sort(decreasing = TRUE)
  }
...
top.taxa <- function (biom, rank = 'OTU', n = Inf) {
  ...
  x <- taxa.sums(biom, rank) %>% names() %>% head(n)
