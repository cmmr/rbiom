...
taxa.rollup <- function (biom, rank='OTU', map=NULL, lineage=FALSE, sparse=FALSE, long=FALSE, md=FALSE, safe=FALSE) {
  ...
  #--------------------------------------------------------------
  # Make the autogenerated columns safe
  #--------------------------------------------------------------
  SampleCol <- "Sample"
  TaxaCol   <- "Taxa"
  if (isTRUE(safe)) {
    colnames(res) <- paste0(".", tolower(colnames(res)))
    SampleCol <- ".sample"
    TaxaCol   <- ".taxa"
  }
  ...
  if (!isFALSE(md)) {
    ...
    if (isTRUE(long)) {
      res <- merge(res, md, by.x = SampleCol, by.y = "row.names")
      ...
      if (isTRUE(safe)) colnames(res)[1] <- ".sample"
      
      res <- merge(md, res, by.x = "row.names", by.y = SampleCol)
      names(res)[1] <- SampleCol
    }
      
  }
  
  if (isTRUE(long))
    if (length(unique(res[[TaxaCol]])) > 1)
      attr(res, 'facet') <- TaxaCol
