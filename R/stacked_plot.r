...
#'                 
#' @param facets   Optionally specify which facets to show and in what order. 
#'        Default: \code{NULL}.
#'                 
...
#'                 
#' @param label.by,sort.by   What metadata column to use for labeling and/or
...
stacked_plot <- function (
    ...
    facet.by = NULL, facets = NULL,
    colors = NULL, patterns = FALSE,
    label.by = NULL, sort.by = NULL, 
  ...
  rank  <- bool_switch(
    test = is.null(rank), 
    yes  = tail(c('OTU', taxa.ranks(biom)), 1), 
  ...
  if (!is.null(sort.by))  sort.by  <- as.vector(validate_metrics(biom, sort.by,  "meta"))
  
  
  #________________________________________________________
  # Inject color.by and pattern.by arguments as needed.
  #________________________________________________________
  if (isFALSE(colors) && isFALSE(patterns)) patterns   <- structure(TRUE,    display = FALSE)
  if (!isFALSE(colors))                     color.by   <- structure(".taxa", display = FALSE)
  if (!isFALSE(patterns))                   pattern.by <- structure(".taxa", display = FALSE)
  ...
  if (length(params[['rank']]) > 1) {
    ...
    plots <- sapply(ranks, simplify = FALSE, function (rank) {
      ...
      do.call(stacked_plot, args)
    })
    
    p <- patchwork::wrap_plots(plots, ncol = 1)
     
    history <- sprintf("stacked_plot(%s)", as.args(params, fun = stacked_plot))
    attr(p, 'history') <- c(attr(biom, 'history'), history)
    attr(p, 'data')    <- plyr::ldply(plots, attr, which = 'data', exact = TRUE) %>% rename_cols('.id' = '.rank')
    attr(p, 'cmd')     <- paste(collapse = "\n\n", local({
      cmds <- sapply(seq_along(ranks), function (i) {
        sub(
          x           = attr(plots[[i]], 'cmd', exact = TRUE), 
          pattern     = "ggplot(data)", 
          replacement = sprintf("p%i <- ggplot(subset(data, .rank = %s))", i, glue::single_quote(ranks[[i]])),
          fixed       = TRUE )
      })
  ...
  #________________________________________________________
  ggdata <- bdply(biom, params[['facet.by']], function (b) {
    
    taxa <- params[['taxa']]
    mat  <- as.matrix(taxa.rollup(b, rank = params[['rank']], sparse = TRUE))
  ...
  #________________________________________________________
  # Order samples on x-axis by similarity or sort.by
  #________________________________________________________
  ...
  setLayer(
    ...
    'caption' = with(params, glue::glue(
      ...
      if (is.null(label.by) && is.null(sort.by)) {
        ...
        "Samples are ordered by {sort.by})*"
        ...
        ifelse(
          test = identical(label.by, sort.by), 
          yes  = "Samples are labeled and ordered by {label.by}.", 
          no   = "Samples are labeled by {label.by} and ordered by {sort.by}." )
      }
    )))
  setLayer(layer = "theme", 'plot.caption' = element_text(face = "italic"))
