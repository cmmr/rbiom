# #' Subset samples using metadata or taxonomy.
# #' 
...
# #' \code{sample_subset(hmp50, mean(Genus) >= 0.1)} and the returned rbiom 
# #' object will contain only the genera that average at least 10% relative 
# #' abundance across all the samples.
# #' 
# #' If you want only orders that are present in three or more samples, you can 
# #' do: \code{sample_subset(hmp50, count(Order) >= 3)}. To require presence in 
# #' 25% of samples, you'd use: 
# #' \code{sample_subset(hmp50, percent(Order) >= 0.25)}.
# #' 
...
# #' 
# #' 
# #' @param biom,x   An rbiom object, as returned from [read_biom()].
# #' 
...
# #'  }
# #'
# sample_subset <- function (biom, expr, env = parent.frame(), drop.na = TRUE, refactor = TRUE) {
# 
...
#   #   if (eq(expr[[1]], as.name("expression")))
#   #     return (do.call(sample_subset, list(biom, expr[[1]], env, drop.na, refactor)))
#   #
#   # if (eq(substitute(expr)[[1]], as.name("expression")))
#   #   return (do.call(sample_subset, list(biom, expr[[1]], env, drop.na, refactor)))
#   #
...
#   #________________________________________________________
#   if (mode == "taxonomy") {
#     envir <- as.list(otu_taxonomy(biom))
# 
# 
#     #________________________________________________________
...
#     #________________________________________________________
#     keep <- tryCatch(
#       expr  = eval(expr, envir = envir),
#       error = function (e) stop("Subset failed: ", e) )
# 
# 
# 
#     otu_taxonomy(biom) <- dplyr::slice(which(keep))
# 
# 
#   } else {
...
#         if (i %in% metadata_names(biom))
#           biom <- sample_select(biom, !is.na(sample_metadata(biom, i)))
# 
# 
# 
#     #________________________________________________________
...
#         levels(biom$metadata[[key]]) %<>% intersect(specs[[key]], .)
#     }
# 
#   }
# 
