...
write_fasta <- function (seqs, outfile) {
  ...
  if (!is(seqs, 'character') || !is(names(seqs), 'character'))
    stop (simpleError('In write_fasta(), seqs must be a named character vector or a BIOM-class object.'))
  
  if (!is.character(outfile) || !identical(nchar(outfile) >= 3, TRUE) || length(outfile) != 1)
    stop (simpleError('In write_fasta(), outfile must be a single filename at least 3 characters in length.'))
