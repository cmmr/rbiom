...
When working with very large datasets, you can make use of these tips
and tricks to speed up operations on rbiom objects.

## Skip Cloning

Functions that modify rbiom objects, like
[`subset()`](https://cmmr.github.io/rbiom/reference/subset.md) and
[`rarefy()`](https://cmmr.github.io/rbiom/reference/rarefy.md), will
automatically clone the object before modifying it. This is to make
these functions behave as most R users would expect - but at a
...
Reference sequences for OTUs will be imported along with the rest of
your dataset and stored in `$sequences`. However, rbiom doesn't
currently use these sequences for anything (except writing them back out
with
...
Alternatively, you can store the tree separately from the rbiom object
and provide it to just the functions that use it. For example:
...
You can also specify a cache directory where results can be preserved
from one R session to the next.
...
However, when you have thousands of data points, rendering every single
one can be slow. Instead, set the `layers` parameter to use other
options.
